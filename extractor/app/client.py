...
class ERPClient:
    ...
    def get_page(
        ...
        page_label = f"{endpoint} | page {page_num}" if page_num else endpoint
    ...
    def fetch_all_pages(self, endpoint: str, updated_after: str | None = None):
        all_rows = []
        cursor = None
        page_num = 1
        ...
        while True:
            ...
            print(f"[{endpoint}] page {page_num}: fetched {len(data)} rows | total: {len(all_rows)}")

            has_more = meta.get("has_more", False)
            cursor = meta.get("cursor")

            if not has_more:
                print(f"[{endpoint}] completed pagination | total rows: {len(all_rows)}")
                break
...
if __name__ == "__main__":
    client = ERPClient()
    payload = client.get_page("/stores/", page_num=1)
    print("Top-level keys:", payload.keys())
    print("Rows returned:", len(payload.get("data", [])))
